...
static event_response_t on_seed_write(vmi_instance_t vmi, vmi_event_t *event){
    ...
    uint8_t out = event->mem_event.out_access;
    // 1) 쓰기가 섞여 있어야만 사용. 아니면 즉시 무시
    if ((out & VMI_MEMACCESS_W) == 0) {
        return 0;
    }
...
static int arm_write_watch(vmi_instance_t vmi, vmi_event_t* ev_out){
    ...
    ev_out->mem_event.in_access = VMI_MEMACCESS_W | VMI_MEMACCESS_X | VMI_MEMACCESS_R;
